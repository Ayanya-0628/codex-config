...
- **版本**: 2.0.0
- **描述**: 基于 AI 自动生成高质�?PPT 图片和视频，支持智能转场和交互式播放
- **作�?*: 歸藏
...
- 🎨 **多风格支�?* - 内置渐变毛玻璃、矢量插画两种专业风�?
- 🖼�?**高质量图�?* - 使用 Nano Banana Pro 生成 16:9 高清 PPT
...
**必需�?*
- `GEMINI_API_KEY`: Google AI API 密钥（用于生�?PPT 图片�?
...
**这是 Skill 的核心优�?*：我（Claude Code）会分析生成�?PPT 图片，为每个转场生成精准的视频提示词�?
...
对于每对相邻图片，我会：
1. **视觉分析**：理解两张图片的布局、元素、色彩差�?
2. **生成预览提示�?*：为首页创建可循环的微动效描�?
3. **生成转场提示�?*：详细描述如何从起始帧过渡到结束�?
...
播放逻辑:
1. 首页: 播放循环预览视频
2. 按右�?�?播放转场视频 �?显示目标页图片（2 秒）
3. 再按右键 �?播放下一个转�?�?显示下一页图�?
4. 依此类推...
...
**1. API 密钥未设�?*
```
错误: ⚠️ 未找�?.env 文件，尝试使用系统环境变�?
      未设�?GEMINI_API_KEY 环境变量
...
解决:
1. 如果只需要图片，跳过视频生成步骤
2. 如果需要视频，配置 KLING_ACCESS_KEY �?KLING_SECRET_KEY
```
...
**生成速度�?*
- PPT 图片：~30 �?页（2K）| ~60 �?页（4K�?
- 转场视频：~30-60 �?�?
- 视频合成：~5-10 �?

**文件大小�?*
- PPT 图片：~2.5MB/页（2K）| ~8MB/页（4K�?
- 转场视频：~3-5MB/段（1080p�? 秒）
- 完整视频：~12-20MB�? �?PPT + 转场�?
...
1. **文档质量**：输入文档内容越清晰结构化，生成�?PPT 质量越高
2. **页数选择**：根据文档长度和演示场景合理选择页数
3. **分辨率选择**：日常使用推�?2K，重要展示场合可�?4K
4. **视频功能**：首次使用建议先尝试仅图片模式，熟悉后再使用视频功能
5. **提示词调�?*：查�?`prompts.json` 了解生成逻辑，可手动调整后重新生�?
...
**用户输入�?*
```
我需要基于这份会议纪要生成一�?5 页的 PPT，使用矢量插画风格�?
...
**用户输入�?*
```
基于 AI-Product-Design.md 文档，生成一�?15 页的 PPT，使用渐变毛玻璃风格，需要转场视频�?
```
...
- 项目架构：参�?`ARCHITECTURE.md`
- API 管理：参�?`API_MANAGEMENT.md`
- 环境配置：参�?`ENV_SETUP.md`
- 安全说明：参�?`SECURITY.md`
- 完整文档：参�?`README.md`
