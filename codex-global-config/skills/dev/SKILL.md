...
- Orchestrate a streamlined 7-step development workflow (Step 0 + Step 1鈥?):
  ...
  5. Coverage validation (鈮?0% requirement)
...
- **Step 4: Parallel Development Execution [CODEAGENT-WRAPPER ONLY - NO DIRECT EDITS]**
  ...
  - Backend routing (must be deterministic and enforceable):
    - Task field: `type: default|ui|quick-fix` (missing 鈫?treat as `default`)
    - Preferred backend by type:
      - `default` 鈫?`codex`
      - `ui` 鈫?`gemini` (enforced when allowed)
      - `quick-fix` 鈫?`claude`
    - If user selected `浠?codex`: all tasks MUST use `codex`
    - Otherwise, if preferred backend is not in `allowed_backends`, fallback to the first available backend by priority: `codex` 鈫?`claude` 鈫?`gemini`
  - Build ONE `--parallel` config that includes all tasks in `dev-plan.md` and submit it once via Bash tool:
    ...
    Test: [test command]
    Deliverables: code + unit tests + coverage 鈮?0% + coverage summary
...
- **Step 5: Coverage Validation**
  - Validate each task鈥檚 coverage:
    - All 鈮?0% 鈫?pass
    - Any <90% 鈫?request more tests (max 2 rounds)
...
- **Backend unavailable**: If a routed backend is unavailable, fallback to another backend in `allowed_backends` (priority: codex 鈫?claude 鈫?gemini); if none works, fail with a clear error message

**Quality Standards**
- Code coverage 鈮?0%
- Tasks based on natural functional boundaries (typically 2-5)
- Each task has exactly one `type: default|ui|quick-fix`
- Backend routed by `type`: `default`鈫抍odex, `ui`鈫抔emini, `quick-fix`鈫抍laude (with allowed_backends fallback)
