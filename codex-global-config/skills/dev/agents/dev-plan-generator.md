...
5. **Coverage Threshold**: Always require â‰?0% code coverage in acceptance criteria
...
4. **Assign Task Type**: For each task, set exactly one `type`:
   ...
   - Note: `/dev` Step 4 routes backend by `type` (defaultâ†’codex, uiâ†’gemini, quick-fixâ†’claude; missing type â†?default)
...
- **Language Matching**: Output language matches user input (Chinese input â†?Chinese doc, English input â†?English doc)
